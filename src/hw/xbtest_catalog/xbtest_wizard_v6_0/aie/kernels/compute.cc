...
v16int16 rdmem0[N_MEM_SIZE];

v8int32  wrmem0[N_MEM_SIZE];
v8int32  wrmem1[N_MEM_SIZE];
v8int32  wrmem2[N_MEM_SIZE];

void compute ( input_stream_int32 * restrict sin0, output_stream_int32 * restrict sout0 ) {
    int i;
    ...
    for(i = 0; i < (16*16*N_MEM_SIZE/32); i++) chess_prepare_for_pipelining {
        tmp0[i]   = 0xFFFFFFFF;
    }
    ...
    while (1) chess_prepare_for_pipelining chess_loop_range(8,) {
        {
            for (i = 0; i < N_KRNL_LOOP; i++) chess_prepare_for_pipelining chess_loop_range(8,) {
                ...
                xstart0+=2; ystart0+=2; writeincr_v4(sout0, srs(ext_lo(acc0_2),shft)); //shft+=4;
                ...
                xstart0+=2; ystart0+=2; writeincr_v4(sout0, srs(ext_lo(acc1_2),shft)); //shft+=4;
            }
        }
    }
}
