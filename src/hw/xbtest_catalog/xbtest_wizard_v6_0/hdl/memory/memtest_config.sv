...
module memtest_config #(
    ...
    parameter integer C_NUM_CU_REGISTER     = 25  // In multi-reg read mode. Memory CU ctrl + status I/F has 25 registers (addr in [0x300000..0x300018))
) (
    // System Signals
    input  wire                             clk,
    input  wire                             rst,
    ...
    output wire [C_PLRAM_ADDR_WIDTH-1:0]    plram_araddr,
    output wire [8-1:0]                     plram_arlen,
...
/////////////////////////////////////////////////////////////////////////////
// FIFO write
logic  [4:0]                        memory_core_id_last     = 'h0;
logic  [LP_CNT_WIDTH-1:0]           memory_core_id          = 'h0;
logic  [4:0]                        memory_core_id_d1       = 'h0;
    ...
    case(current_state)
            ...
            if (start_pulse) begin
                ...
                case (scalar00[1:0])
                    ...
                    default : begin // Read multiple reg
                        ...
                        addr[26:22]         <= 'h0;                         // memory CU core index
                        addr[21]            <= 1'b1;                        // memory CU core register select
                        addr[20]            <= 1'b1;                        // memory CU core register select
                        addr[19:0]          <= C_NUM_CU_REGISTER-1; // memory CU core register offset
        ...
        ST_NEXT_RD_CU: begin
            if (plram_FIFO_wr_ptr[$high(plram_FIFO_wr_ptr)]) begin
                ...
                memory_core_id_d1       <= memory_core_id[4:0];
                current_state           <= ST_WAIT_WR_PLRAM;
            end else begin
                cs                      <= 1'b1;
                addr[19:0]              <= plram_FIFO_wr_ptr[19:0];
                current_state           <= ST_WAIT_RD_CU;
            end
        end // ST_NEXT_RD_CU
        ...
        ST_WAIT_WR_PLRAM: begin
            if (plram_write_done) begin
                if (memory_core_id_d1 == memory_core_id_last) begin
                    ...
                    addr                <= 'h0;
                    addr[27]            <= 1'b1;                        // memory CU cores
                    addr[26:22]         <= memory_core_id[4:0];         // memory CU core index
                    addr[21]            <= 1'b1;                        // memory CU core register select
                    addr[20]            <= 1'b1;                        // memory CU core register select
                    addr[19:0]          <= C_NUM_CU_REGISTER-1;         // memory CU core register offset
