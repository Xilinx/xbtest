...
wire                                ctrl_update_cfg_pulse;
wire [1:0]                          ctrl_test_mode;
wire                                ctrl_reset;
wire                                ctrl_clear_err_pulse;
...
wire [31:0]                     rdata_int;
wire                            cmd_cmplt_int;
wire [C_M_AXI_ADDR_WIDTH-1:0]   axi_addr_ptr_int;
// TODO: need to add a CU ready signal as these go through CDC resync
wire                            toggle_1_sec_0;

//   [$size(cs)+$size(we)+$size(addr)+$size(wdata)+$size(rdata)+$size(cmd_cmplt)+$size(axi_addr_ptr)+$size(toggle_1_sec)-1:0]
wire [1        +1        +22         +32          +32          +1               +C_M_AXI_ADDR_WIDTH +1                  -1:0] pipeline_in, pipeline_out;

assign pipeline_in = {  cs,
                        we,
                        addr,
                        wdata,
                        rdata_int,
...
pipeline #(
    .C_NUM_PIPELINE ( C_NUM_PIPELINE        ),
    .C_DATA_WIDTH   ( $size(pipeline_in)    )
) pipeline_if (
    .clk        ( ap_clk        ),
    .rst        ( ap_rst        ),
    .data_in    ( pipeline_in   ),
...
always_ff @(posedge ap_clk) begin
    ...
    stat_toggle_1_sec   <= toggle_1_sec_0;

    if (stat_toggle_1_sec ^ toggle_1_sec_0) begin
...
memtest_reg_array #(
    ...
    .wdata                          ( wdata_int                     ),
    .rdata                          ( rdata_int                     ),
    .cmd_cmplt                      ( cmd_cmplt_int                 ),
    .watchdog_alarm                 ( watchdog_alarm                ),

    .ctrl_stop_pulse                ( ctrl_stop_pulse               ),
    .ctrl_update_cfg_pulse          ( ctrl_update_cfg_pulse         ),
    .ctrl_test_mode                 ( ctrl_test_mode                ),
    .ctrl_reset                     ( ctrl_reset                    ),
    .ctrl_clear_err_pulse           ( ctrl_clear_err_pulse          ),
    ...
    .stat_cfg_updated_pulse         ( stat_cfg_updated_pulse        ),
    ...
    .stat_axi_addr_ptr              ( axi_addr_ptr_int              ),
