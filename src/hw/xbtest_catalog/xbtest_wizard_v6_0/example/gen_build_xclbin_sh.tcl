...
proc gen_build_xclbin_sh { config_ref } {
    ...
    foreach cu_cfg [dict get $config cu_config] {
        ...
        if {($krnl_mode == 3) || ($krnl_mode == 4) || ($krnl_mode == 6)} {
            set serial_port  [dict get $cu_cfg serial_port]
            set diff_clocks  [dict get $cu_cfg diff_clocks]

            log_message $config {GEN_BUILD_XCLBIN_SH-3} [list $kernel_name serial_port $serial_port]
    ...
    #           vitis_analyzer ${XBTEST_RUN_DIR}/aie_compile/graph.aiecompile_summary
    ...
    #           aiesimulator --pkg-dir ${XBTEST_RUN_DIR}/aie_compile --simulation-cycle-timeout=10000
    #############################################################################################################
    if {$use_aie} {
        lappend BUILD_XCLBIN_SH [log_message_bash {INFO} {XBTEST_WIZARD-27} "Executing aiecompiler. Full console output captured in \${XBTEST_RUN_DIR}/aie_compile.console.log"]
        ...
        lappend BUILD_XCLBIN_SH "--include=\${XBTEST_IPDEF}/aie/data \\";                       # All the include files needed to build the graph
        lappend BUILD_XCLBIN_SH "--include=\${XILINX_HLS}/include  \\";                         # All the include files needed to build the graph
        lappend BUILD_XCLBIN_SH "--include=\${XILINX_VITIS_AIETOOLS}/include  \\";              # All the include files needed to build the graph
        lappend BUILD_XCLBIN_SH "--pl-freq=$clock0_freq \\";                                    # Specify frequency (in MHz) for all PL kernels and PLIOs.
        lappend BUILD_XCLBIN_SH "--Xpreproc=-DPL_FREQ_MHZ=$clock0_freq \\";                     # The PL freq to be passed to PLIO constructor; Maybe it's redundant with --pl-freq
        lappend BUILD_XCLBIN_SH "--Xpreproc=-DNUMCORES=$num_aie_used \\";                       # The number of kernel to be generated in graph
        lappend BUILD_XCLBIN_SH "--workdir=\${XBTEST_RUN_DIR}/aie_compile \\";                  # The location of where the Work directory will be created
    ...
    if {$vpp_link_output == {default}} {
        ...
        if {([dict get $config tool_version] >= 2022.1) && ([regexp {xcvc} [dict_get_quiet $config {wizard_actual_config platform fpga_part}]] == 1)} {
    ...
    if {$use_aie} {
        lappend BUILD_XCLBIN_SH "\${XBTEST_RUN_DIR}/libadf.a \\"
    }
    ...
    if {![dict get $config STOP_POST_OPT]} {
        ...
        lappend BUILD_XCLBIN_SH "-i $sh_tmp_xclbin \\"
        lappend BUILD_XCLBIN_SH "-o $sh_output_xclbin \n"; # end of command
    ...
    lappend VPP_LINK_INI "platform=$xpfm"
    ...
    if {$use_aie || ($vpp_link_output == {xsa})} {
        ...
        lappend VPP_PACKAGE_INI "platform=$xpfm"
    ...
    if {[dict exists $config gt_mac_definition] || [dict exists $config gt_lpbk_definition] || [dict exists $config gt_prbs_definition]} {
        ...
        set gt_idx              0
        set gt_idx_max          [expr [llength $gt_mac_def_list] + [llength $gt_lpbk_def_list] + [llength $gt_prbs_def_list] - 1]
