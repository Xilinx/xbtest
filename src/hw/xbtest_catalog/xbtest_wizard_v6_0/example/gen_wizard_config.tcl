...
proc log_wizard_config { config cfg_name {wizard_config_names_in {}} {filename_in {}} } {
    ...
    foreach wizard_config_name $wizard_config_names {
        ...
        if {($platform_fpga_part != {}) || ($platform_name != {}) || ($platform_interface_uuid != {}) || ($platform_is_nodma != {}) || ($platform_p2p_support != {}) || ($platform_mac_addresses_available != {}) || ($platform_gt != {})} {
        ...
        if {($cu_configuration_clock != {}) || ($cu_configuration_plram_selection != {}) || ($cu_configuration_verify != {}) || ($cu_configuration_gt != {})
         || ($cu_configuration_gt_mac != {}) || ($cu_configuration_memory != {}) || ($cu_configuration_power != {})        } {
            set is_cu_configuration_section true
        } else {
            set is_cu_configuration_section false
        }
        if {($cu_selection_power != {}) || ($cu_selection_gt_mac != {}) || ($cu_selection_gt_lpbk != {}) || ($cu_selection_gt_prbs != {}) || ($cu_selection_memory != {})} {
    ...
    lappend CONFIG_JSON     "$TAB_2[string2json {platform}] : \{"
            if {$platform_fpga_part != {}} {
                if {($platform_name != {}) || ($platform_interface_uuid != {}) || ($platform_is_nodma != {}) || ($platform_p2p_support != {}) || ($platform_mac_addresses_available != {}) || ($platform_gt != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3[string2json {fpga_part}] : [string2json $platform_fpga_part]$next"
            }
            if {$platform_name != {}} {
                if {($platform_interface_uuid != {}) || ($platform_is_nodma != {}) || ($platform_p2p_support != {}) || ($platform_mac_addresses_available != {}) || ($platform_gt != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3[string2json {name}] : [string2json $platform_name]$next"
            }
            if {$platform_interface_uuid != {}} {
                if {($platform_is_nodma != {}) || ($platform_p2p_support != {}) || ($platform_mac_addresses_available != {}) || ($platform_gt != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3[string2json {interface_uuid}] : [string2json $platform_interface_uuid]$next"
            }
            if {$platform_is_nodma != {}} {
                if {($platform_p2p_support != {}) || ($platform_mac_addresses_available != {}) || ($platform_gt != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3[string2json {is_nodma}] : $platform_is_nodma$next"
            }
            if {$platform_p2p_support != {}} {
                if {($platform_mac_addresses_available != {}) || ($platform_gt != {})} { set next "," } else { set next "" }
    ...
    lappend CONFIG_JSON             "$TAB_4\}$next"
                ...
                if {($cu_configuration_plram_selection != {}) || ($cu_configuration_verify != {}) || ($cu_configuration_gt != {}) || ($cu_configuration_gt_mac != {}) || ($cu_configuration_memory != {}) || ($cu_configuration_power != {})} { set next "," } else { set next "" }
    ...
    lappend CONFIG_JSON             "$TAB_4[string2json $slr] : [string2json $sptag]$next"
                ...
                if {($cu_configuration_verify != {}) || ($cu_configuration_gt != {}) || ($cu_configuration_gt_mac != {}) || ($cu_configuration_memory != {}) || ($cu_configuration_power != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3\}$next"
            }
    ...
    lappend CONFIG_JSON             "$TAB_4[string2json {dna_read}] : $dna_read"
                ...
                if {($cu_configuration_gt != {}) || ($cu_configuration_gt_mac != {}) || ($cu_configuration_memory != {}) || ($cu_configuration_power != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3\}$next"
            }
    ...
    lappend CONFIG_JSON             "$TAB_4\}$next"
                ...
                if {($cu_configuration_gt_mac != {}) || ($cu_configuration_memory != {}) || ($cu_configuration_power != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3\}$next"
            }
    ...
    lappend CONFIG_JSON     "$TAB_2[string2json {cu_selection}] : \{"
            if {$cu_selection_power != {}} {
                if {($cu_selection_gt_mac != {}) || ($cu_selection_gt_lpbk != {}) || ($cu_selection_gt_prbs != {}) || ($cu_selection_memory != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3[string2json {power}] : [list2json $cu_selection_power]$next"
            }
            if {$cu_selection_gt_mac != {}} {
                if {($cu_selection_gt_lpbk != {}) || ($cu_selection_gt_prbs != {}) || ($cu_selection_memory != {})} { set next "," } else { set next "" }
    lappend CONFIG_JSON         "$TAB_3[string2json {gt_mac}] : [list2json $cu_selection_gt_mac]$next"
            }
            if {$cu_selection_gt_lpbk != {}} {
                if {($cu_selection_gt_prbs != {}) || ($cu_selection_memory != {})} { set next "," } else { set next "" }
...
proc load_power_floorplan_json { config_ref } {
    ...
    if {![file exists $dynamic_geometry_json]} {
        ...
        set supported_keys {COMMENT PART PBLOCKS SLR CLOCK_REGIONS SITES_TYPES CONFIG_SITES PL_DYNAMIC_GEOMETRY AIE_GEOMETRY}
