...
#########################################################################################
proc get_CONFIG_SITES { SITES_TYPES CLOCK_REGIONS dynamic_pblocks} {
    set CONFIG_SITES {}
...
#########################################################################################
proc gen_dynamic_geometry { pwr_floorplan_dir dynamic_pblocks } {
    ...
    foreach ulp_pblock $dynamic_pblocks {
        if { [llength [lsearch -all $dynamic_pblocks $ulp_pblock]] > 1 } {
    ...
    set dsp48e2_found 0
    foreach ulp_pblock $dynamic_pblocks {
        if {[llength [get_sites -quiet -filter [subst {NAME =~ ${SITES_TYPE_DSP48E2}_X* && !PROHIBIT}] -of [get_pblocks $ulp_pblock]]] > 0} {
            set dsp48e2_found 1
        }
    }
    ...
    set aie_found 0
    foreach ulp_pblock $dynamic_pblocks {
        if {[llength [get_sites -quiet -filter [subst {NAME =~ ${SITES_TYPE_AIE}_X* && !PROHIBIT}] -of [get_pblocks $ulp_pblock]]] > 0} {
    ...
    set CONFIG_SITES        [get_CONFIG_SITES        $SITES_TYPES $CLOCK_REGIONS $dynamic_pblocks]
    set PL_DYNAMIC_GEOMETRY [get_PL_DYNAMIC_GEOMETRY $SITES_TYPES $CLOCK_REGIONS $dynamic_pblocks]
    if {$aie_found} {
        set AIE_GEOMETRY [get_AIE_GEOMETRY $SITE_KEY_AIE $SITES_TYPE_AIE $CLOCK_REGIONS $dynamic_pblocks]
    }
    ...
    lappend DYNAMIC_GEOMETRY_JSON "$TAB_1[string2json {PBLOCKS}] : [strlist2json $dynamic_pblocks],"
    # Write SLR
...
# vivado -mode tcl -source /proj/ipeng/staff/cedricg/Perforce/IP3_cedricg_xni_dev/DEV/hw/xbtest_wizard/xbtest_wizard_v6_0/tcl/power/gen_dynamic_geometry.tcl -tclargs /proj/ipeng/staff/cedricg/xbtestv6/20211103_release/hw_sources/xbtest_wizard/xbtest_wizard_v6_0/xclbin_generate/output/xilinx_u55c_gen3x16_xdma_2_202110_1/1103_release/u_1103_release_ex/run/vpp_link/link/vivado/vpl/prj/prj.runs/impl_1/level0_wrapper_opt.dcp ./pwr_cfg/ pblock_dynamic_SLR0 pblock_dynamic_SLR1
